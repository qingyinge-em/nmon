...
void gpu_init()
{
    ...
    if ((nvres = nvmlSystemGetDriverVersion(&gpu_driver_version[0],
             1024))
        != NVML_SUCCESS) {
        printf("nvmlSystemGetDriverVersion failed %d\n", nvres);
        return;
    }
    if ((nvres = nvmlSystemGetNVMLVersion(&gpu_nvml_version[0],
             1024))
        != NVML_SUCCESS) {
        printf("nvmlSystemGetDriverVersion failed %d\n", nvres);
        return;
        ...
        while (fgets(tmpstr, LSCPU_STRLEN, pop) != NULL) {
		   ...
		   0123456789012345678901 
            ...
            if (strncmp("Byte Order:", tmpstr, strlen("Byte Order:")) == 0) {
                ...
                if (strncmp("Little Endian", lscpu.byte_order,
                        strlen("Little Endian"))
                    == 0) {
                    lscpu.order = ORDER_LITTLE;
                }
                if (strncmp("Big Endian", lscpu.byte_order,
                        strlen("Big Endian"))
                    == 0) {
            ...
            if (strncmp("Thread(s) per core:", tmpstr,
                    strlen("Thread(s) per core:"))
                == 0) {
                sscanf(&tmpstr[data_col], "%d", &lscpu.threads);
            }
            if (strncmp("Core(s) per socket:", tmpstr,
...
void get_intel_spec()
{
    ...
    for (i = 0; i < proc[P_CPUINFO].lines; i++) {
        if ((strncmp("bogomips", proc[P_CPUINFO].line[i], 8) == 0) || (strncmp("bogoMIPS", proc[P_CPUINFO].line[i], 8) == 0) || (strncmp("BogoMIPS", proc[P_CPUINFO].line[i], 8) == 0)) {
...
void proc_cpu()
{
    ...
    if (intr_line == 0) {
        if (proc[P_STAT].line[i + 1][0] == 'p' && proc[P_STAT].line[i + 1][1] == 'a' && proc[P_STAT].line[i + 1][2] == 'g' && proc[P_STAT].line[i + 1][3] == 'e') {
            /* 2.4 kernel */
            intr_line = i + 3;
            ctxt_line = i + 5;
            btime_line = i + 6;
...
*/
    for (i = 0; i < DISKMAX;) {
        ...
        p->dk[i].dk_major = p->dk[i].dk_minor = p->dk[i].dk_name[0] = p->dk[i].dk_reads = p->dk[i].dk_rmerge = p->dk[i].dk_rkb = p->dk[i].dk_rmsec = p->dk[i].dk_writes = p->dk[i].dk_wmerge = p->dk[i].dk_wkb = p->dk[i].dk_wmsec = p->dk[i].dk_inflight = p->dk[i].dk_time = p->dk[i].dk_backlog = 0;
...
long proc_mem_search(char* s)
{
    ...
    for (i = 0; i < proc[P_MEMINFO].lines; i++) {
        if (!strncmp(s, proc[P_MEMINFO].line[i], len)) {
            for (j = len;
                 !isdigit(proc[P_MEMINFO].line[i][j]) && proc[P_MEMINFO].line[i][j] != 0; j++)
                /* do nothing */;
...
void hint(void)
{
    printf("\t-e            Exclude tun interfaces in net monitor\n\n\n");
...
*/
    for (i = 0; i < NETMAX; /* i++ */) {
        ...
        if (exclude_tun && strstr((char*)&p->ifnets[i].if_name, "tun")) {
            continue;
        }
...
#define DISPLAY(pad, rows)                                                              \
    ...
    while (-1 != (i = getopt(argc, argv, "?abc:C:Dd:eEfF:g:hI:Jl:m:MNpr:Rs:tTUVxXz"))) {
        switch (i) {
        case '?':
            hint();
            exit(0);
            ...
            if (welcome && getenv("NMON") == 0) {
                ...
                case VM_POWERVM:
                    ...
                    mvwprintw(padwelcome, x + 9, 3, "PowerVM %s %s",
                        &proc[P_CPUINFO].line[1][7],
                        &proc[P_CPUINFO].line[proc[P_CPUINFO].lines - 1][11]);
                    mvwprintw(padwelcome, x + 10, 3,
...
*/
                    ...
                    if (lparcfg.shared_processor_mode == 1) {
                        if (lparcfg.timebase == -1) {
                            ...
                            for (i = 0; i < proc[P_CPUINFO].lines - 1; i++) {
                                if (!strncmp("timebase", proc[P_CPUINFO].line[i],
                                        8)) {
        ...
        if (show_lpar) {
            ...
            if (cursed) {
                ...
                } else if (power_vm_type == VM_POWERKVM_HOST
                    || power_vm_type == VM_POWERKVM_GUEST) {
        ...
        if (show_gpu) {
            ...
            for (i = 0; i < gpu_devices; i++) {
                if (cursed && first_time_gpu) {
                    if (nvmlDeviceGetName(gpu_device[i], &gpu_name[i][0],
                            1024)
                        != NVML_SUCCESS)
                ...
                if (nvmlDeviceGetTemperature(gpu_device[i], NVML_TEMPERATURE_GPU,
                        &gpu_temp[i])
                    != NVML_SUCCESS)
                    gpu_temp[i] = 999;
                if (nvmlDeviceGetPowerUsage(gpu_device[i], &gpu_watts[i])
                    != NVML_SUCCESS)
                    gpu_watts[i] = 999000;
                if (nvmlDeviceGetClockInfo(gpu_device[i], NVML_CLOCK_GRAPHICS,
                        &gpu_clock[i])
                    != NVML_SUCCESS)
                    gpu_clock[i] = 999;
            }
...
#define DATACOL 11
		 ...
		 */
                ...
                for (lineno = 0; lineno < proc[P_CPUINFO].lines; lineno++) {
                    if (strncmp(clockstr, proc[P_CPUINFO].line[lineno],
                            strlen(clockstr))
                        == 0) {
                        if (show_mhz == 1
                ...
                if (lparcfg.cmo_enabled != 0) {
                    if (!show_rrd)
                        fprintf(fp,
                            ...
                            /* three zeros here */
                            (float)(lparcfg.backing_memory) / 1024 / 1024, lparcfg.cmo_page_size / 1024,
...
#define NFS_DELTA(member) (((double)(p->member - q->member) / elapsed))
                ...
                if (nfs_v2c_found) {
                    fprintf(fp,
                        show_rrd ? "rrdtool update nfscliv2.rrd %s" : "NFSCLIV2,%s", LOOP);
                    for (i = 0; i < 18; i++) {
                ...
                if (nfs_v2s_found) {
                    fprintf(fp,
                        show_rrd ? "rrdtool update nfsvrv2.rrd %s" : "NFSSVRV2,%s", LOOP);
                    for (i = 0; i < 18; i++) {
                        fprintf(fp, show_rrd ? ":%.1f" : ",%.1f",
                ...
                if (nfs_v3c_found) {
                    fprintf(fp,
                        show_rrd ? "rrdtool update nfscliv3.rrd %s" : "NFSCLIV3,%s", LOOP);
                    for (i = 0; i < 22; i++) {
                        fprintf(fp, show_rrd ? ":%.1f" : ",%.1f",
                ...
                if (nfs_v3s_found) {
                    fprintf(fp,
                        show_rrd ? "rrdtool update nfsvrv3.rrd %s" : "NFSSVRV3,%s", LOOP);
                    for (i = 0; i < 22; i++) {
                        fprintf(fp, show_rrd ? ":%.1f" : ",%.1f",
                ...
                if (nfs_v4c_found) {
                    fprintf(fp,
                        show_rrd ? "rrdtool update nfscliv4.rrd %s" : "NFSCLIV4,%s", LOOP);
                    for (i = 0; i < nfs_v4c_names_count; i++) {
                        fprintf(fp, show_rrd ? ":%.1f" : ",%.1f",
                ...
                if (nfs_v4s_found) {
                    fprintf(fp,
                        show_rrd ? "rrdtool update nfsvrv4.rrd %s" : "NFSSVRV4,%s", LOOP);
                    for (i = 0; i < nfs_v4c_names_count; i++) {
                        fprintf(fp, show_rrd ? ":%.1f" : ",%.1f",
...
*/
            ...
            if (!cursed) {
                ...
                fprintf(fp, "\n");
                fprintf(fp,
                    show_rrd ? "rrdtool update netpacket.rrd %s" : "NETPACKET,%s", LOOP);
                for (i = 0; i < networks; i++) {
                    fprintf(fp, show_rrd ? ":%.1f" : ",%.1f",
                        IFDELTA(if_ipackets));
                }
            ...
            if (cursed) {
                ...
                for (k = 0, j = 0; k < JFSMAX && j < jfses; k++) {
                    ...
                    if (!strncmp(jfs[k].name, "/proc/", 6) /* sub directorys have to be fake too */
                        ...
                        || !strncmp(jfs[k].name, "/rpc_pipe", 10)) { /* /proc gives invalid/insane values */
                ...
                jfs_load(LOAD);
                fprintf(fp,
                    show_rrd ? "rrdtool update jfsfile.rrd %s" : "JFSFILE,%s", LOOP);
                for (k = 0; k < jfses; k++) {
                    if (jfs[k].mounted && strncmp(jfs[k].name, "/proc", 5)
                        && strncmp(jfs[k].name, "/sys", 4)
                        && strncmp(jfs[k].name, "/dev/", 5)
...
*/
                ...
                for (i = 0; i < disks; i++) {
                    if (NEWDISKGROUP(i))
                        fprintf(fp,
                            show_rrd ? "%srrdtool update diskbusy%s.rrd %s" : "%sDISKBUSY%s,%s", i == 0 ? "" : "\n",
                            dskgrp(i), LOOP);
...
*/
                ...
                if (dgroup_loaded == 2) {
                    fprintf(fp,
                        show_rrd ? "rrdtool update dgbusy.rdd %s" : "DGBUSY,%s", LOOP);
                    ...
                    fprintf(fp, "\n");
                    fprintf(fp,
                        show_rrd ? "rrdtool update dgread.rdd %s" : "DGREAD,%s", LOOP);
                    for (k = 0; k < dgroup_total_groups; k++) {
                        if (dgroup_name[k] != 0) {
                            disk_total = 0.0;
                            for (j = 0; j < dgroup_disks[k]; j++) {
...
*/
                    ...
                    fprintf(fp, "\n");
                    fprintf(fp,
                        show_rrd ? "rrdtool update dgwrite.rdd %s" : "DGWRITE,%s", LOOP);
                    for (k = 0; k < dgroup_total_groups; k++) {
                        if (dgroup_name[k] != 0) {
                            disk_total = 0.0;
                            for (j = 0; j < dgroup_disks[k]; j++) {
...
*/
                    ...
                    fprintf(fp, "\n");
                    fprintf(fp,
                        show_rrd ? "rrdtool update dgbsize.rdd %s" : "DGSIZE,%s", LOOP);
                    for (k = 0; k < dgroup_total_groups; k++) {
                        if (dgroup_name[k] != 0) {
                            disk_write = 0.0;
                            disk_xfers = 0.0;
...
*/
                    ...
                    fprintf(fp, "\n");
                    fprintf(fp,
                        show_rrd ? "rrdtool update dgxfer.rdd %s" : "DGXFER,%s", LOOP);
                    for (k = 0; k < dgroup_total_groups; k++) {
                        if (dgroup_name[k] != 0) {
                            disk_total = 0.0;
                            for (j = 0; j < dgroup_disks[k]; j++) {
                ...
                case 1: /* Basic */
                    if (cursed) {
                        ...
                        for (j = 0; j < max_sorted; j++) {
                            ...
                            if (!show_all
                                && (topper[j].time / elapsed < ignore_procdisk_threshold) && !cmdfound)
                                break;
